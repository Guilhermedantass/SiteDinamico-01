...
class Painel
{
    ...
    {
        if ($tipo == 'sucesso') {
            echo '<div class="box-alert sucesso"><p> ' . $mensagem . '</p> </div>';
        } else if ($tipo == 'erro') {
            echo '<div class="box-alert erro"><p>' . $mensagem . '</p></div> ';
        }
    }
    ...
    public static function orderItem($tabela, $orderType, $idItem)
    {
        if ($orderType == 'up') {
            ...
            $infoItemAtual = Painel::select($tabela, 'id=?', array($idItem));
            $order_id = $infoItemAtual['order_id'];
            $itemBefore = MySql::conectar()->prepare("SELECT * FROM `$tabela` WHERE order_id > $order_id  ORDER BY order_id LIMIT 1");
            $itemBefore->execute();
            if ($itemBefore->rowCount() == 0) {
                return;
            }
